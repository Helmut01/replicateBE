...
where $\small{\theta_0=\mu_\textrm{T}/\mu_\textrm{R}}$ and the Null hypothesis is <u>in</u>equivalence. In Average Bioequivalence the limits $\small{[L,U]}$ are fixed, whereas in ABEL they can be [expanded](#BElim) based on the variability of the reference treatment.
...
```
In full replicate designs (required by the WHO for reference-scaling of *AUC*; see [below](#app_out)) the same model is run with `data = data[data$treatment = "T", ]`.
