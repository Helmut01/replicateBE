...
##### Example of different data sets{#setexpl}
16 subjects included. In sequence `RTRT` one dropout in the 2^nd^ period and one in the 4^th^ period. In sequence `TRTR` one dropout in the 3^rd^ period and one in the 4^th^.\
...
We obtain two data sets:

1. 13 subjects for the estimation of _CV~wR~_ (insufficient data of subjects 6, 8, and 14).
2. 15 subjects who received at least one treatment `T` and `R` for the assessment of BE (all except subject 8).
...
With the additional argument `ola = TRUE` in `method.A()` and `method.B()` an outlier analysis is performed, where the default `fence = 2`^[The fences are the lowest data value still within _m_×IQR of the lower quartile, and the highest value still within _m_×IQR of the upper quartile, where IQR is the interquartile range (difference between the 3^rd^ and 1^st^ quartiles). Data outside fences are considered outliers. Decreasing the multiplier _m_ to _e.g._, 1.5 might result in many outliers whereas increasing the multiplier in only a few.  
Note that different methods exist to calculate quartiles (seven are available in R, where the default is `type = 7`). R’s default is used by S+ and Excel. Phoenix/WinNonlin, Minitab, and SPSS use `type = 6`, whereas the default in SAS is `type = 2` (though others are available as well).] is used.
...
If based on studentized residuals outliers are detected, additionally to the scaled limits based on the complete reference data, tighter limits are calculated based on _CV~wR~_ after exclusion of outliers and BE assessed with the new limits.
