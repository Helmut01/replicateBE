...
Relevant data are used for the estimation of _CV~wR~_ (and _CV~wT~_ in full replicate designs) and BE, _i.e._, the data sets might be different. It is good practice to state that in the SAP.
...
```
For informational purposes in full replicate designs (required by the WHO for reference-scaling of AUC) the same model is additionaly run with `data = data[data$treatment = "T", ]`.
...
Note that different methods exist to calculate quartiles (nine are available in R, where the default is `type = 7`). R’s default is used by S, MATLAB, and Excel. Phoenix/WinNonlin, Minitab, and SPSS use `type = 6`, ScyPy uses `type = 4`, whereas the default in SAS and Stata is `type = 2` (though others are available as well).] is used.  

Note that results might slightly differ depending on software’s algorithms. Example with methods implemented in R:
```{r}
